...
private:
	...
	static size_t randomInRange(size_t _min, size_t _max);
	/// @returns a random number between 0 and _weights.size() - 1, where i has
	/// probability _weights[i] / sum of weights
	static size_t weightedRandom(std::vector<size_t> const& _weights);
	static bool probabilityPercentage(size_t _percentage);
	static std::vector<std::string> randomlyChooseFrom(std::vector<std::string> const& _names, size_t _amount);
	...
	static int const s_maxExpressionDepth = 6;
