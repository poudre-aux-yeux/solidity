...
Block RandomCodeGenerator::generateBlock(int _blockDepth)
{
	...
	for (size_t i = 0; i < statements; i++)
	{
		try
		{
			switch (weightedRandom({3, 2, 2, 1, 1, 1, 1, 1}))
			{
			case 0:
...
FunctionDefinition RandomCodeGenerator::generateFunctionDefinition(int _blockDepth)
{
	...
	funDef.returnVariables = generateAndRegisterVariables(numParams);
...
Switch RandomCodeGenerator::generateSwitch(int _blockDepth)
{
	...
	for (size_t i = 0; i < numCases; ++i)
		switchStatement.cases.emplace_back(Case{
			{},
			make_shared<Literal>(generateLiteral()),
...
{
	// TODO randomize depth

	// FunctionalInstruction, FunctionCall, Identifier, Literal
	vector<size_t> weights{3, 2, 1, 1};
	if (_expressionDepth < 2)
		weights[0] = weights[1] = 1;
	else if (_expressionDepth <= 1)
	...
	for (size_t i = 0; i < 6; i++)
	{
		try
		{
			...
			{
			case 0: return generateFunctionalInstruction(_returnValues, _expressionDepth);
			case 1: return generateFunctionCall(_returnValues, _expressionDepth);
...
FunctionalInstruction RandomCodeGenerator::generateFunctionalInstruction(size_t _returnValues, int _expressionDepth)
{
	...
	if (weightsReturnOne.empty() || weightsReturnZero.empty())
	{
		...
		for (auto const& w: vector<tuple<solidity::Instruction, size_t>>{
				{solidity::Instruction::ADD, 6},
				{solidity::Instruction::SUB, 4},
				{solidity::Instruction::MUL, 4},
				{solidity::Instruction::DIV, 4},
				{solidity::Instruction::SDIV, 4},
				{solidity::Instruction::MOD, 4},
				{solidity::Instruction::SMOD, 4},
				{solidity::Instruction::EXP, 4},
				{solidity::Instruction::NOT, 6},
				{solidity::Instruction::LT, 4},
				{solidity::Instruction::GT, 4},
				{solidity::Instruction::SLT, 4},
				{solidity::Instruction::SGT, 4},
				{solidity::Instruction::EQ, 4},
				{solidity::Instruction::ISZERO, 4},
				{solidity::Instruction::AND, 4},
				{solidity::Instruction::OR, 4},
				{solidity::Instruction::XOR, 4},
				{solidity::Instruction::BYTE, 3},
				{solidity::Instruction::SHL, 4},
				{solidity::Instruction::SHR, 4},
				{solidity::Instruction::ADDMOD, 4},
				{solidity::Instruction::MULMOD, 4},
				{solidity::Instruction::SIGNEXTEND, 4},
				{solidity::Instruction::KECCAK256, 3},
				...
				{solidity::Instruction::CREATE2, 2},
		})
		...
		for (auto const& w: vector<tuple<solidity::Instruction, size_t>>{
			...
			{solidity::Instruction::MSTORE, 4},
			{solidity::Instruction::MSTORE8, 4},
			{solidity::Instruction::SSTORE, 4},
...
{
	// TODO extend this.
	static vector<string> const literals{"0", "1", "2", "3", "0xff", "0xff0000000000000000", "0x10001"};
	Literal lit;
...
size_t RandomCodeGenerator::randomInRange(size_t _min, size_t _max)
{
	...
	return std::uniform_int_distribution<size_t>{_min, _max}(engine);
}
